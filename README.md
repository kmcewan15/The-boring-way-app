...
- The animation is native `scrollTo({ behavior: 'smooth' })`. It still emits
...
[images/](images/) holds every raster and standalone SVG file in the project. None
of it is wired into the app — the running UI draws its artwork inline from the
components above, so this folder is a working store: reference exports, rendered
previews, and debug screenshots kept from earlier passes. Drop new image assets
here. Anything you actually want the app to load needs a `public/` folder instead,
which Vite serves at the site root.
