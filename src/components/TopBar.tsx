...
export default function TopBar({
  ...
  /* Closing the drawer destroys whatever was focused inside it, which drops
     focus to <body> and restarts the next Tab at the top of the document. Hand
     it back to the button that opened it. Skipped on the first run, so loading
     the app does not steal focus. */
  const wasOpen = useRef(false);
  useEffect(() => {
    if (wasOpen.current && !drawerOpen) menuRef.current?.focus();
  ...
  return (
    <header className="topbar">
      ...
      <NavDrawer open={drawerOpen} onClose={() => onOpenChange(false)} />
    </header>
  );
}
