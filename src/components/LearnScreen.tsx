...
export default function LearnScreen({
  ...
  /* True while an overlay covers the stage. The trail stays mounted behind one,
     and its key handlers are on `window`, so without this an arrow press walked
     the hidden trail and you came back several steps from where you left. */
  blocked?: boolean;
}) {
  ...
  /* The map lives with the trail rather than in App, so opening it needs no prop
     ...
     second `m`. Declared up here because the wheel and arrow handlers below both
     have to stand down while it is open. */
  ...
  const goTo = useCallback((i: number) => {
    ...
    /* Landing in a different world means somebody picked it rather than scrolled
       to it, so go straight there. Gliding the 28 pages from topic 2 to topic 8
       crossed six worlds on the way, and each crossing fired its own arrival
       banner and rebuilt the landscape -- a strobe, not a journey.
       Distance is the wrong test: the "Current step" pill jumps two to four
       entries and is usually still inside one world, where the glide is the
       whole point. */
    const far = JOURNEY[next].topic.number !== JOURNEY[parked.current].topic.number;
    parked.current = next;
    lastStepAt.current = performance.now();
    rail.scrollTo({ top: next * rail.clientHeight, behavior: far ? 'auto' : 'smooth' });
  }, []);
  ...
  /* A world picked from the nav drawer, which can be opened -- and can pick a
     ...
     so this reads as "arrive, then go to the picked world" rather than a fight
     over where the rail starts. The second move is a cut, not a glide, because
     a picked world is always a different one -- see `far` in goTo. */
  ...
  /* A page is one rail-height, and `scrollTop` survives a resize in pixels, so
     changing the window height leaves the rail parked between two pages -- a
     618px rail that becomes 508px turns page 1 into page 1.217, and the cards sit
     half a step out. Put it back on the page we were on. */
  useEffect(() => {
    const onResize = () => {
      const rail = railRef.current;
      if (!rail || rail.clientHeight === 0) return;
      rail.scrollTo({ top: parked.current * rail.clientHeight, behavior: 'auto' });
    };
  ...
  useEffect(() => {
    ...
    if (!stage) return;
    /* The map renders inside `.trail`, so this listener sits above it and would
       otherwise scroll the trail behind the open map. */
    if (blocked || mapOpen) return;

    const onWheel = (e: WheelEvent) => {
      if (e.ctrlKey) return; // pinch-to-zoom is the browser's business

      /* Ignore jitter and sideways swipes: the trail only runs up and down. The
         test comes before preventDefault, so a two-finger sideways swipe is still
         the browser's back gesture rather than something we swallow. */
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      e.preventDefault();
      /* One step per press, held or not. The OS repeats a held key about thirty
         times a second, which flew the whole trail and strobed an arrival banner
         for every world on the way. `repeat` is the whole guard: a real second
         press is a real second step, so unlike the wheel this path throttles
         nothing. */
      if (e.repeat) return;
  ...
  return (
    <section className="trail" ref={stageRef}>
      {/* Every other screen has a heading; this one had none, because the world
          it is showing is named in the top bar and drawn into the scenery. Given
          to a screen reader only, so the trail is still a picture and not a page
          with a title stamped on it. */}
      <h1 className="sr-only">
        Topic {focusTopic.number} of {TOPICS.length}: {focusTopic.title}
      </h1>
